...
export async function getPropCollection(
  directory,
...
export async function getContentCollection(directory, isReverse = true) {
  const directoryFullPath = path.join(process.cwd(), directory);
  const fileNames = fs.readdirSync(directoryFullPath);
  const collection = await Promise.all(
    fileNames.map(async (fileName) => {
...
export async function getDirectorySlugs(directory) {
  const directoryFullPath = path.join(process.cwd(), directory);
